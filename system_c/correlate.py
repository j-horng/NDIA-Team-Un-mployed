...
def orb_ransac_georeg(
    cam: ImageFrame,
    sat_gray,
    sat_pix2geo: Callable[[float, float], tuple],
    nfeatures=2000,
    fast=12,
    min_inliers=40,
    ransac_px=3.0,
    ...
    h, w = cam_gray.shape[:2]
    center = np.array([[[w / 2.0, h / 2.0]]], dtype=np.float32)
    sat_center = cv2.perspectiveTransform(center, H)[0, 0]
    lon, lat = sat_pix2geo(float(sat_center[0]), float(sat_center[1]))
    reproj = cv2.perspectiveTransform(p1[None, inl, :], H)[0]
    rmse = float(np.sqrt(np.mean((reproj - p2[inl]) ** 2)))
    conf = float(
        min(1.0, (0.5 + 0.5 * inlier_ratio) * (1.0 / (1.0 + rmse / 3.0)))
    )
